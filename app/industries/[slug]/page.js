...
async function resolveIndustryData(slug) {
  const [blobIndustries, blobServices, blobCaseStudies] = await Promise.all([
  ...
  return { industry: industry || getIndustryBySlug(slug), services, caseStudies };
}
...
export default async function IndustryDetailPage({ params }) {
  const { slug } = await params;
  const { industry, services: blobServices, caseStudies } = await resolveIndustryData(slug);

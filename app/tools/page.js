...
export default async function ToolsPage() {
  let dbTools = []
  try { dbTools = await getCachedCollection('tools') } catch {}
  const allTools = dbTools.length ? dbTools : staticTools
...
function ToolCard({ tool }) {
  return (
    <Link
      ...
      className="group relative flex flex-col bg-card border border-divider rounded-2xl p-6 hover:border-accent/40 card-glow transition-all duration-200"
    >
      {tool.badge && (
      ...
      <div className="flex items-center gap-1.5 text-accent-light text-sm font-semibold group-hover:gap-2.5 transition-all">

    ...
    <>
      ...
      <div className="flex flex-col gap-4">
        {loading ? <div className="text-gray-400 text-sm">Loading…</div> :
          data.map((faq) => (
            <div key={faq.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 flex flex-col gap-3">
              <div className="flex items-start gap-3">
                <div className="flex-1 flex flex-col gap-2">
                  <input value={faq.question} onChange={(e) => update(faq.id, { question: e.target.value })}
                    className="w-full text-sm font-medium border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    placeholder="Question" />
                  <textarea value={faq.answer} onChange={(e) => update(faq.id, { answer: e.target.value })}
                    rows={3} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    placeholder="Answer" />
                </div>

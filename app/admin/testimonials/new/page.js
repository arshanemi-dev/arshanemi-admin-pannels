...
export default function NewTestimonialPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (res.ok) { addToast('Testimonial added!'); router.push('/admin/testimonials') }
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto">
      <PageHeader title="Add Testimonial" backHref="/admin/testimonials" />
          ...
          <div className="flex gap-1">
            {[1, 2, 3, 4, 5].map((n) => (
              <button key={n} type="button" onClick={() => setForm((f) => ({ ...f, rating: n }))}>
                <Star className={`w-6 h-6 transition-colors ${n <= form.rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} />
              </button>
            ))}
          </div>
        </div>

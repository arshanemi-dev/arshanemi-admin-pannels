...
export default function TeamPage() {
  ...
  return (
    <>
      <PageHeader title="Team Members" newHref="/admin/team/new" />
      {loading ? <div className="text-gray-400 text-sm">Loading…</div> : (
        <DataTable columns={columns} data={data} searchKeys={['name', 'role']}
          actions={(row) => (
            <div className="flex gap-2 justify-end">
              <Link href={`/admin/team/${row.id}`} className="p-1.5 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50"><Edit className="w-4 h-4" /></Link>

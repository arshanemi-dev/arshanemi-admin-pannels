...
- **Three trigger points**, matching what was asked:
  ...
  3. **Platform-wide sweep — `scripts/cron-reconcile-topups.mjs`** (`npm run cron:reconcile`), **not** an HTTP route. Direct instruction, after confirming the app is hosted on Vercel: `node-cron` cannot run inside the Vercel-hosted app itself — serverless functions don't stay alive between requests, so an in-process schedule would silently stop firing, the same problem the original `setInterval` idea had. This script is a standalone, always-on Node process (same convention as the pre-existing `scripts/cron-cleanup.mjs`) that talks to Supabase and Razorpay directly — no HTTP hop into the Next.js app, so no secret needed for it. **It must be run somewhere other than Vercel** — a small VPS, Docker, PM2, a Railway/Render worker service, etc. — Vercel itself will not host this process. `app/api/cron/reconcile-topups/route.js` and `vercel.json` (the earlier Vercel-Cron-based version of this) were removed along with the now-unneeded `CRON_SECRET`; `lib/paymentReconciliation.js` is unaffected — it's still used by the user-scoped `/api/wallet/reconcile` route (trigger points 1 and 2 above) — the standalone script duplicates its logic rather than importing it, since a plain Node script can't resolve the app's `@/` path alias.

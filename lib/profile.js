...
// get `null` here rather than an extra round trip.
export function serializeProfile(user, company, walletExpiresAt = null) {
  ...
  return {
    ...
    // Unclamped — a metered charge (e.g. the storage-billing cron) can push
    // this negative (debt); clamping here would silently hide that from the
    // user and understate the admin's aggregate exposure. See
    // deduct_wallet_coins' p_allow_negative in storage_billing_migration.sql.

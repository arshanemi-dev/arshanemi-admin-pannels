...
// Server-to-server auth for standalone tool apps (e.g. tools/arshanemi-tools-1
// running with NEXT_PUBLIC_IS_CONNECT=true) that have no admin browser
// session/cookie to forward — just a shared secret set on both sides. Scoped
// to the handful of routes that opt in explicitly (files-expiry); every other
// admin-only route keeps using getAdminFromRequest's real JWT check.
export async function getAdminOrServiceFromRequest(req) {

...
export async function createCoinPackage({ name, coins, pricePaise, badge = null, isActive = true, displayOrder = 0 }) {
  const supabase = getSupabase()
  const { data, error } = await supabase
    .from('coin_packages')
    .insert({ name, coins, price_paise: pricePaise, badge, is_active: isActive, display_order: displayOrder })
    .select()
    .single()
  if (error) throw error
  return coinPackageRowToItem(data)
...
export async function createWalletTopup({ userId, coinPackageId, packageName, amountPaise, coinsGranted, razorpayOrderId }) {
  const supabase = getSupabase()
  const { data, error } = await supabase
    .from('wallet_topups')
    .insert({

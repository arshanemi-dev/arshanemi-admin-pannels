...
// ─── Tools Access (per-user tool grants, stored as the 'toolsAccess' singleton) ─

export async function getToolsAccessMap() {
  return (await getSingleton('toolsAccess')) ?? {}
}

export async function grantAllToolsAccess(userId, toolSlugs) {
  const map = await getToolsAccessMap()
  map[userId] = toolSlugs
  await updateSingleton('toolsAccess', map)
  return map[userId]
}

...
export async function createUser({ name, email, mobile, passwordHash, role = 'user', companyId = null, otpEnabled = false }) {
  const supabase = getSupabase()
  const { data, error } = await supabase
    .from('users')
    .insert({ name, email, mobile, password_hash: passwordHash, role, company_id: companyId, otp_enabled: otpEnabled })
    .select()
    .single()
  if (error) throw error
  return data
...
// (used by the company-scoped 'admin' role).
export async function getAllUsers({ companyId } = {}) {
  const supabase = getSupabase()
  let query = supabase
    .from('users')
    .select('id,name,email,mobile,role,company_id,is_active,otp_enabled,created_at')

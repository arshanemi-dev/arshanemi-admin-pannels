...
// Memoized — every exported function below calls getSupabase() on every
// invocation, and creating a fresh SupabaseClient each time spins up its own
// GoTrueClient bound to the same storage key, which is what triggers
// Supabase's "Multiple GoTrueClient instances detected" warning. One client
// instance is reused for the lifetime of the server process instead.
...
// access to, returned as a Map of `${toolSlug}::${featureApiIdentifier}` →
// {isActive, nextRenewalAt} for O(1) lookups while enriching the response.
export async function getFeatureActivations(userId, toolSlugs) {
  if (!toolSlugs?.length) return new Map()
  const supabase = getSupabase()
  const { data, error } = await supabase
    .from('feature_activations')
    .select('tool_slug, feature_api_identifier, is_active, next_renewal_at')
    .eq('user_id', userId)
    .in('tool_slug', toolSlugs)
  if (error) throw error
  return new Map(
    (data ?? []).map((row) => [
      `${row.tool_slug}::${row.feature_api_identifier}`,
      { isActive: row.is_active, nextRenewalAt: row.next_renewal_at },
    ])
  )
}

// Coin-based recurring activation — see scripts/feature_activation_migration.sql.
// Composes deduct_wallet_coins with the activation upsert atomically; the
// authoritative server-side gate used by POST /api/wallet/deduct.
export async function activateFeature({ userId, toolId, toolSlug, featureId, featureApiIdentifier, featureTitle, coins, idempotencyKey = null }) {
  const supabase = getSupabase()
  const { data, error } = await supabase.rpc('activate_feature', {
    p_user_id: userId, p_tool_id: toolId, p_tool_slug: toolSlug,
    p_feature_id: featureId, p_feature_api_identifier: featureApiIdentifier,
    p_feature_title: featureTitle, p_coins: coins, p_idempotency_key: idempotencyKey,
  })
  if (error) throw error
  return data
}

export async function deactivateFeature({ userId, toolSlug, featureApiIdentifier }) {
  const supabase = getSupabase()
  const { data, error } = await supabase.rpc('deactivate_feature', {
    p_user_id: userId, p_tool_slug: toolSlug, p_feature_api_identifier: featureApiIdentifier,
  })
  if (error) throw error
  return data
}
...
// quantity is audit-only (how many units this deduction covers, e.g. files
// in one upload batch) — the caller is responsible for having already
// multiplied it into `amount`. allow_negative is intentionally NOT a param
// here — that flag only exists on the RPC for the storage-billing cron's
// direct supabase.rpc() call, never reachable from this Next.js-side wrapper.
export async function deductWalletCoins({ userId, amount, toolId, toolSlug, featureId, featureApiIdentifier, featureTitle, idempotencyKey = null, quantity = null }) {
  const supabase = getSupabase()
  const { data, error } = await supabase.rpc('deduct_wallet_coins', {
    ...
    p_feature_id: featureId, p_feature_api_identifier: featureApiIdentifier,
    p_feature_title: featureTitle, p_idempotency_key: idempotencyKey, p_quantity: quantity,
  })
  if (error) throw error
  return data
}

export async function reportStorageUsage({ userId, provider, deltaBytes, idempotencyKey = null }) {
  const supabase = getSupabase()
  const { data, error } = await supabase.rpc('report_storage_usage', {
    p_user_id: userId, p_provider: provider, p_delta_bytes: deltaBytes, p_idempotency_key: idempotencyKey,
  })
  if (error) throw error
  return data
}

// Summed across every provider row (dropbox + bunny) — used by
// lib/toolAccess.js to decide whether a user is still within the Storage
// feature's free allowance before forcing them through the Activate gate.
export async function getTotalStorageBytesForUser(userId) {
  const supabase = getSupabase()
  const { data, error } = await supabase
    ...
    .eq('user_id', userId)
  if (error) throw error
  return (data ?? []).reduce((sum, row) => sum + (row.storage_bytes ?? 0), 0)
}

// ─── SKU Master & Mappings (per-user — tools/arshanemi-tools-2, 'pdf-cropper') ─
// Backs GET/POST/DELETE /api/sku/{master,map}, used only when that app runs
// with NEXT_PUBLIC_IS_CONNECT=true (see scripts/sku_mapping_migration.sql).
// Each function returns the caller's fresh full list, matching the shape
// that app's lib/skuStore.js already expects from its local blob-store route.
...
// the old name — same two-step behavior as tools-2's local blob-store route.

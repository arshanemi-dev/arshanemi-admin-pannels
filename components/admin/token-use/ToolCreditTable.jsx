// "Tools Name / Credit Use" list — the bottom half of the Token Use panel.
export default function ToolCreditTable({ tools }) {
  return (
      ...
      <table className="w-full min-w-[420px] border-collapse">
        <thead>
          <tr className="bg-surface">
            ...
            <th className="px-5 py-3.5 text-left text-sm font-bold text-foreground">Credit Use</th>
          </tr>
        </thead>
        <tbody>
          {tools.map((tool) => (
            <tr key={tool.name} className="border-t border-divider hover:bg-surface transition-colors">
              ...
              <td className="px-5 py-3.5 text-sm text-muted">{tool.creditUse}</td>
            </tr>
          ))}
        </tbody>
      </table>
